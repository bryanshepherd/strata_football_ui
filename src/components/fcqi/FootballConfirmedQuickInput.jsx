...
export const getFootballFcqiAssistantMessage = (state) => {
  ...
  if (state.currentStep === 'fieldGoalNextSpot') return 'Confirm or change the next ball spot, prefilled from the previous line of scrimmage.';
...
export default function FootballConfirmedQuickInput({
  ...
  const startGameControl = (startedBy) => {
    // Final games expose roster corrections; scoring transitions stay locked.
  ...
  const commitToken = (value) => {
    ...
    if ((settingsOnlyMenuOpen || currentState.currentStep === 'gameControlMenu') && normalizedValue === 'R' && onOpenStarters) {
      setSettingsOnlyMenuOpen(false);
      clearModalStepHistory();
      publishState(createInitialFootballQuickInputState());
      onOpenStarters();
  ...
  return (
    <section className="rounded border border-zinc-300 bg-white">
      ...
      <FootballFlowModal
        ...
        startersEditable={Boolean(onOpenStarters)}
